...
function openMomentsPage(userId = null, backToMain = false) {
    try {
        ...
        // ★ 右上角：隐藏原来的 + 按钮，用「…」统一收纳发布和规则
const publishBtn = document.getElementById('momentsPublishBtn');
publishBtn.style.display = 'none'; // 隐藏原来的 + 按钮

const headerRight = publishBtn.parentElement;
let menuBtn = document.getElementById('momentsMenuBtn');
if (!menuBtn) {
    menuBtn = document.createElement('span');
    menuBtn.id = 'momentsMenuBtn';
    menuBtn.style.cssText = 'cursor:pointer;color:var(--theme);display:flex;align-items:center;';
    menuBtn.innerHTML = `<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="5" r="2"/><circle cx="12" cy="12" r="2"/><circle cx="12" cy="19" r="2"/></svg>`;
    menuBtn.title = '更多';
...
menuBtn.onclick = (e) => {
    e.stopPropagation();
    // 关闭已打开的菜单
    const existing = document.querySelector('.moments-menu-popup');
    if (existing) { existing.remove(); return; }
    
    const popup = document.createElement('div');
    popup.className = 'moments-menu-popup';
    popup.style.cssText = 'position:absolute;top:48px;right:8px;z-index:200;background:var(--secondary-bg);border-radius:12px;box-shadow:0 4px 16px rgba(0,0,0,0.15);padding:6px;display:flex;flex-direction:column;min-width:130px;';
    
    // 发布动态
    const publishItem = document.createElement('button');
    publishItem.className = 'btn';
    publishItem.style.cssText = 'display:flex;align-items:center;gap:8px;padding:10px 14px;font-size:14px;border:none;background:none;cursor:pointer;width:100%;text-align:left;';
    publishItem.innerHTML = `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>发布动态`;
    publishItem.onclick = () => { popup.remove(); showMomentPublish(); };
    popup.appendChild(publishItem);
    
    // 朋友圈规则（仅MJ朋友圈显示）
if (currentMomentUserId !== 'me') {
    const ruleItem = document.createElement('button');
    ruleItem.className = 'btn';
    ruleItem.style.cssText = 'display:flex;align-items:center;gap:8px;padding:10px 14px;font-size:14px;border:none;background:none;cursor:pointer;width:100%;text-align:left;';
    ruleItem.innerHTML = `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="4" y1="21" x2="4" y2="14"/><line x1="4" y1="10" x2="4" y2="3"/><line x1="12" y1="21" x2="12" y2="12"/><line x1="12" y1="8" x2="12" y2="3"/><line x1="20" y1="21" x2="20" y2="16"/><line x1="20" y1="12" x2="20" y2="3"/><line x1="1" y1="14" x2="7" y2="14"/><line x1="9" y1="8" x2="15" y2="8"/><line x1="17" y1="16" x2="23" y2="16"/></svg>朋友圈规则`;
    ruleItem.onclick = () => { popup.remove(); showMomentRuleDialog(currentMomentUserId); };
    popup.appendChild(ruleItem);
}

// ★ 清空朋友圈
const clearItem = document.createElement('button');
clearItem.className = 'btn';
clearItem.style.cssText = 'display:flex;align-items:center;gap:8px;padding:10px 14px;font-size:14px;border:none;background:none;cursor:pointer;width:100%;text-align:left;color:#f44336;';
clearItem.innerHTML = `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#f44336" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>清空朋友圈`;
clearItem.onclick = () => {
    popup.remove();
    const targetName = currentMomentUserId === 'me' ? '我的' : (getContact(currentMomentUserId)?.name || '');
    showConfirm(`清空${targetName}的所有朋友圈动态？此操作不可恢复`, () => {
        appData.moments[currentMomentUserId] = [];
        markDataChanged();
        save();
        renderMomentsList();
        toast('朋友圈已清空');
...
function renderMomentsList() {
    ...
    [...moments].reverse().forEach((m, i) => {
        ...
        const hasLiked = m.likedBy && m.likedBy.includes('me');
      const likeColor = hasLiked ? '#f44336' : 'var(--theme)';
      
      const likeSpan = document.createElement('span');
      likeSpan.style.cssText = 'display:inline-flex;align-items:center;gap:2px;cursor:pointer;';
      likeSpan.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="${hasLiked ? '#f44336' : 'none'}" stroke="${likeColor}" stroke-width="2">${getIconSVG('heart')}</svg> ${m.likes || 0}`;
      likeSpan.onclick = () => window.likeMoment(realIdx);
      
      const commentSpan = document.createElement('span');
      commentSpan.style.cssText = 'display:inline-flex;align-items:center;gap:2px;cursor:pointer;';
      commentSpan.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--theme)" stroke-width="2">${getIconSVG('message-circle')}</svg> ${(m.comments || []).length}`;
      commentSpan.onclick = () => window.commentMoment(realIdx); actions.appendChild(likeSpan); actions.appendChild(commentSpan); div.appendChild(actions);
        if ((m.comments || []).length) {
            ...
            m.comments.forEach((c, ci) => {
                ...
                const delSpan = document.createElement('span'); delSpan.style.cssText = 'cursor:pointer;color:#f44336;margin-left:4px'; delSpan.innerHTML = `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#f44336" stroke-width="2" style="vertical-align:middle;pointer-events:none;">${getIconSVG('trash-2')}</svg>`;
                delSpan.style.cssText = 'cursor:pointer;margin-left:4px;display:inline-flex;align-items:center;'; delSpan.onclick = () => window.deleteComment(realIdx, ci);
                cDiv.appendChild(delSpan); commentsDiv.appendChild(cDiv);
            });
            div.appendChild(commentsDiv);
        }
        const delMomentSpan = document.createElement('span'); delMomentSpan.className = 'moment-delete'; delMomentSpan.style.cssText = 'cursor:pointer;color:#f44336;display:flex;align-items:center;';
delMomentSpan.innerHTML = `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#f44336" stroke-width="2" style="pointer-events:none;">${getIconSVG('trash-2')}</svg>`; delMomentSpan.onclick = () => window.deleteMoment(realIdx);
...
function showMomentPublish() {
    ...
    const closeBtn = document.createElement('span'); closeBtn.className = 'close-pop'; closeBtn.innerHTML = `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">${getIconSVG('x')}</svg>`; closeBtn.onclick = () => m.classList.remove('show');
    ...
    const imgRow = document.createElement('div'); const addImgBtn = document.createElement('button'); addImgBtn.className = 'btn'; addImgBtn.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="margin-right:4px;vertical-align:middle;">${getIconSVG('plus')}</svg>图片`;
    addImgBtn.style.cssText = 'display:inline-flex;align-items:center;';
...
// ========== 朋友圈规则设置弹窗 ==========
function showMomentRuleDialog(userId) {
    ...
    header.textContent = u.name + ' · 朋友圈规则';
    ...
    enabledRow.style.cssText = 'display:flex;align-items:center;justify-content:space-between;';
    enabledRow.innerHTML = '<span style="font-size:14px;">自动发朋友圈</span>';
    const enabledToggle = document.createElement('input');
    enabledToggle.type = 'checkbox';
    enabledToggle.checked = r.enabled;
    enabledToggle.onchange = () => { r.enabled = enabledToggle.checked; };
    enabledRow.appendChild(enabledToggle);
    body.appendChild(enabledRow);

    // 内容拼几条字卡
const combineTitle = document.createElement('div');
combineTitle.style.cssText = 'font-size:13px;font-weight:bold;color:var(--text-secondary);';
combineTitle.textContent = '每条动态拼几条字卡';
body.appendChild(combineTitle);
const combineRow = document.createElement('div');
combineRow.style.cssText = 'display:flex;align-items:center;gap:8px;';
const combineMinInput = document.createElement('input');
combineMinInput.type = 'number'; combineMinInput.value = r.combineMin; combineMinInput.min = 1; combineMinInput.max = 10;
combineMinInput.style.cssText = 'width:60px;padding:4px;border-radius:8px;border:1px solid var(--border-light);text-align:center;';
const combineMaxInput = document.createElement('input');
combineMaxInput.type = 'number'; combineMaxInput.value = r.combineMax; combineMaxInput.min = 1; combineMaxInput.max = 10;
combineMaxInput.style.cssText = 'width:60px;padding:4px;border-radius:8px;border:1px solid var(--border-light);text-align:center;';
combineRow.appendChild(combineMinInput);
combineRow.appendChild(document.createTextNode('~'));
combineRow.appendChild(combineMaxInput);
combineRow.appendChild(document.createTextNode('条字卡'));
body.appendChild(combineRow);

    // 间隔时间（支持秒/分钟/小时）
const intervalTitle = document.createElement('div');
intervalTitle.style.cssText = 'font-size:13px;font-weight:bold;color:var(--text-secondary);margin-top:4px;';
intervalTitle.textContent = '触发间隔（测试建议1-10秒，日常建议30分钟起）';
body.appendChild(intervalTitle);

// 将当前存储的秒数转换为显示值
function secToDisplay(totalSec) {
    if (totalSec >= 3600 && totalSec % 3600 === 0) return { val: totalSec / 3600, unit: 'h' };
    if (totalSec >= 60 && totalSec % 60 === 0) return { val: totalSec / 60, unit: 'm' };
    return { val: totalSec, unit: 's' };
}
const minDisplay = secToDisplay(r.intervalMinSec || 1800);
const maxDisplay = secToDisplay(r.intervalMaxSec || 14400);

// 最小值行
const intervalMinRow = document.createElement('div');
intervalMinRow.style.cssText = 'display:flex;align-items:center;gap:6px;margin-bottom:4px;';
intervalMinRow.appendChild(document.createTextNode('最短'));
const intervalMinInput = document.createElement('input');
intervalMinInput.type = 'number'; intervalMinInput.value = minDisplay.val; intervalMinInput.min = 1; intervalMinInput.max = 999999;
intervalMinInput.style.cssText = 'width:70px;padding:4px;border-radius:8px;border:1px solid var(--border-light);text-align:center;';
intervalMinRow.appendChild(intervalMinInput);
const minUnitSelect = document.createElement('select');
minUnitSelect.style.cssText = 'padding:4px;border-radius:8px;border:1px solid var(--border-light);font-size:12px;';
['s','m','h'].forEach(u => { const o = document.createElement('option'); o.value = u; o.textContent = u === 's' ? '秒' : u === 'm' ? '分钟' : '小时'; if (u === minDisplay.unit) o.selected = true; minUnitSelect.appendChild(o); });
intervalMinRow.appendChild(minUnitSelect);
body.appendChild(intervalMinRow);

// 最大值行
const intervalMaxRow = document.createElement('div');
intervalMaxRow.style.cssText = 'display:flex;align-items:center;gap:6px;';
intervalMaxRow.appendChild(document.createTextNode('最长'));
const intervalMaxInput = document.createElement('input');
intervalMaxInput.type = 'number'; intervalMaxInput.value = maxDisplay.val; intervalMaxInput.min = 1; intervalMaxInput.max = 999999;
intervalMaxInput.style.cssText = 'width:70px;padding:4px;border-radius:8px;border:1px solid var(--border-light);text-align:center;';
intervalMaxRow.appendChild(intervalMaxInput);
const maxUnitSelect = document.createElement('select');
maxUnitSelect.style.cssText = 'padding:4px;border-radius:8px;border:1px solid var(--border-light);font-size:12px;';
['s','m','h'].forEach(u => { const o = document.createElement('option'); o.value = u; o.textContent = u === 's' ? '秒' : u === 'm' ? '分钟' : '小时'; if (u === maxDisplay.unit) o.selected = true; maxUnitSelect.appendChild(o); });
intervalMaxRow.appendChild(maxUnitSelect);
body.appendChild(intervalMaxRow);

    // 触发概率
    const probTitle = document.createElement('div');
    probTitle.style.cssText = 'font-size:13px;font-weight:bold;color:var(--text-secondary);margin-top:4px;';
    probTitle.textContent = '触发概率';
    body.appendChild(probTitle);
    const probRow = document.createElement('div');
    probRow.style.cssText = 'display:flex;align-items:center;gap:8px;';
    const probSlider = document.createElement('input');
    probSlider.type = 'range'; probSlider.min = '1'; probSlider.max = '100';
    probSlider.value = Math.round(r.probability * 100);
    probSlider.style.cssText = 'flex:1;height:6px;-webkit-appearance:none;appearance:none;background:var(--theme-light);border-radius:3px;outline:none;';
    const probLabel = document.createElement('span');
    probLabel.style.cssText = 'min-width:40px;text-align:center;font-size:13px;';
    probLabel.textContent = Math.round(r.probability * 100) + '%';
    probSlider.oninput = () => { probLabel.textContent = probSlider.value + '%'; };
    ...
    limitTitle.style.cssText = 'font-size:13px;font-weight:bold;color:var(--text-secondary);margin-top:4px;';
    limitTitle.textContent = '每天上限';
    body.appendChild(limitTitle);
    const limitRow = document.createElement('div');
    limitRow.style.cssText = 'display:flex;align-items:center;gap:8px;';
    const limitInput = document.createElement('input');
    limitInput.type = 'number'; limitInput.value = r.dailyLimit; limitInput.min = 0; limitInput.max = 999;
    limitInput.style.cssText = 'width:80px;padding:4px;border-radius:8px;border:1px solid var(--border-light);text-align:center;';
    limitRow.appendChild(limitInput);
    limitRow.appendChild(document.createTextNode('条/天（0=不限）'));
    ...
    saveBtn.onclick = () => {
        r.combineMin = Math.max(1, Math.min(10, parseInt(combineMinInput.value) || 1));
        r.combineMax = Math.max(r.combineMin, Math.min(10, parseInt(combineMaxInput.value) || 3));
        // 间隔转秒
        function toSec(val, unit) { if (unit === 'h') return val * 3600; if (unit === 'm') return val * 60; return val; }
        const minSec = toSec(Math.max(1, parseInt(intervalMinInput.value) || 30), minUnitSelect.value);
        const maxSec = toSec(Math.max(1, parseInt(intervalMaxInput.value) || 240), maxUnitSelect.value);
        r.intervalMinSec = minSec;
        r.intervalMaxSec = Math.max(minSec, maxSec);
        r.probability = parseInt(probSlider.value) / 100;
        r.dailyLimit = parseInt(limitInput.value) || 0;
        markDataChanged(); save();
overlay.remove();
// 立即杀掉这个MJ的旧定时器，按新规则重新安排
const user = getContact(userId);
if (window._mjMomentTimers && window._mjMomentTimers[userId]) {
    clearTimeout(window._mjMomentTimers[userId]);
    delete window._mjMomentTimers[userId];
}
if (typeof scheduleMomentForUser === 'function') {
    scheduleMomentForUser(user);
}
toast('朋友圈规则已保存');
    };
    ...
    resetBtn.style.cssText = 'color:#f44336;margin-top:4px;width:100%;';
...
function importContactData(userId, data, mode) {
    ...
    if (mode === 'overwrite') {
        ...
        if (data.momentRule != null) u.momentRule = data.momentRule;  // null 或 undefined 都跳过
    ...
    if (data.momentRule && !u.momentRule) {
      u.momentRule = data.momentRule;
    }
...
// ========== 模拟朋友圈数据 ==========
    ...
    // 获取某个 MJ 的朋友圈规则（带全局默认值）
    function getMomentRule(user) {
      const def = {
        enabled: true,
        combineMin: 1,
        combineMax: 3,
        intervalMinSec: 1800, // 默认最短间隔 30 分钟（秒）
        intervalMaxSec: 14400, // 默认最长间隔 4 小时（秒）
        probability: 0.04,
        dailyLimit: 5
      };
      if (!user || !user.momentRule) return def;
      const r = user.momentRule;
      return {
        enabled: r.enabled !== undefined ? r.enabled : def.enabled,
        combineMin: r.combineMin || def.combineMin,
        combineMax: r.combineMax || def.combineMax,
        intervalMinSec: r.intervalMinSec || def.intervalMinSec,
        intervalMaxSec: r.intervalMaxSec || def.intervalMaxSec,
        probability: r.probability !== undefined ? r.probability : def.probability,
        dailyLimit: r.dailyLimit || def.dailyLimit
      };
    }
    // 为单个 MJ 安排下一次发圈（独立循环）
function scheduleMomentForUser(user) {
    // 清理这个 MJ 的旧定时器
    if (window._mjMomentTimers && window._mjMomentTimers[user.id]) {
        clearTimeout(window._mjMomentTimers[user.id]);
        delete window._mjMomentTimers[user.id];
    }

    const rule = getMomentRule(user);
    if (!rule.enabled) return;

    // 检查今天是否已达上限
    const todayCount = (appData.dailyMomentCount && appData.dailyMomentCount[user.id]) || 0;
    if (todayCount >= rule.dailyLimit) return;

    // 计算延迟（秒转毫秒）
    const minMs = (rule.intervalMinSec || 1800) * 1000;
    const maxMs = (rule.intervalMaxSec || 14400) * 1000;
    const delay = minMs + Math.random() * Math.max(0, maxMs - minMs);

    if (!window._mjMomentTimers) window._mjMomentTimers = {};
    window._mjMomentTimers[user.id] = setTimeout(() => {
        const currentRule = getMomentRule(user);
        if (!currentRule.enabled) return;
        const currentCount = (appData.dailyMomentCount && appData.dailyMomentCount[user.id]) || 0;
        if (currentCount >= currentRule.dailyLimit) return;

        // 概率判定
        if (Math.random() >= currentRule.probability) {
            scheduleMomentForUser(user);
            return;
        }
        ...
        const cardCount = currentRule.combineMin + Math.floor(Math.random() * (currentRule.combineMax - currentRule.combineMin + 1));
        const selectedCards = [];
        ...
        const text = selectedCards.length > 0 ? selectedCards.join(' ') : '今天天气真好';
        ...
        appData.dailyMomentCount[user.id] = (appData.dailyMomentCount[user.id] || 0) + 1;
        markDataChanged(); save();
...
function bindEvents() { createMusicFloatElements(); createBackupFloatElements(); document.getElementById('chatBackBtn').onclick = () => { switchToChatsTab(); renderChatList(); }; document.getElementById('sendMsgBtn').onclick = () => sendMessage(); document.getElementById('messageInput').addEventListener('keypress', e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendMessage(); } }); document.getElementById('emojiTriggerBtn').onclick = () => { const ep = document.getElementById('emojiPanel'); const ip = document.getElementById('imagePanel'); ep.classList.toggle('show'); ip.classList.remove('show'); ep.classList.add('active-panel'); ip.classList.remove('active-panel'); }; document.getElementById('imageUploadBtn').onclick = () => { if (!currentChatId) { toast('请先选择联系人'); return; } const ip = document.getElementById('imagePanel'); const ep = document.getElementById('emojiPanel'); ip.classList.toggle('show'); ep.classList.remove('show'); ip.classList.add('active-panel'); ep.classList.remove('active-panel'); renderImagePanel(); }; document.getElementById('chatMenuBtn').onclick = openChatMenu; document.getElementById('callBtn').onclick = () => { if (!currentChatId) toast('请先选择联系人'); else startCall(); }; document.getElementById('toggleToolsBtn').onclick = () => { toggleToolsBar(); toggleSecondRow(); }; document.getElementById('toggleToolsRowBtn').onclick = toggleToolsRow; document.getElementById('musicTriggerBtn').onclick = toggleMusicFloat; document.getElementById('backupTriggerBtn').onclick = toggleBackupFloat; document.getElementById('shareMovieBtn').onclick = () => { const movies = appData.movieLists.flatMap(l => l.movies); if (movies.length > 0) { shareMovieWithList(); } else { showInputDialog('分享电影', '输入电影名（可选导演、年份）', (val) => { if (val) sendMessage(`🎬 ${appData.myProfile.name} 分享了一部电影《${val}》`); }); } }; document.getElementById('shareBookBtn').onclick = () => { const books = appData.bookLists.flatMap(l => l.books); if (books.length > 0) { shareBookWithList(); } else { showInputDialog('分享书单', '输入书名（可选作者）', (val) => { if (val) sendMessage(`📖 ${appData.myProfile.name} 分享了一本书《${val}》`); }); } }; document.getElementById('shareMusicBtn').onclick = () => { showInputDialog('分享音乐', '输入歌名（可选歌手）', (val) => { if (val) sendMessage(`🎶 ${appData.myProfile.name} 分享了一首歌《${val}》`); }); }; document.getElementById('voiceBtn').onclick = () => { _isVoiceMode = !_isVoiceMode; const input = document.getElementById('messageInput'); const voiceArea = document.getElementById('voiceRecordArea'); const voiceBtn = document.getElementById('voiceBtn'); if (_isVoiceMode) { if (!voiceStream) { navigator.mediaDevices.getUserMedia({ audio: { echoCancellation: true, noiseSuppression: true, autoGainControl: false } }).then(stream => { voiceStream = stream; }).catch(() => { toast('无法访问麦克风，请换用Via浏览器'); _isVoiceMode = false; input.style.display = ''; voiceArea.style.display = 'none'; voiceBtn.innerHTML = '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="M6 8h.01M10 8h.01M14 8h.01M18 8h.01M8 12h.01M12 12h.01M16 12h.01M6 16h12"/></svg>'; return; }); } input.style.display = 'none'; voiceArea.style.display = 'flex'; voiceBtn.innerHTML = '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z"/><path d="M19 10v2a7 7 0 0 1-14 0v-2"/><line x1="12" y1="19" x2="12" y2="23"/><line x1="8" y1="23" x2="16" y2="23"/></svg>'; voiceBtn.title = '切换键盘'; } else { input.style.display = ''; voiceArea.style.display = 'none'; voiceBtn.innerHTML = '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="M6 8h.01M10 8h.01M14 8h.01M18 8h.01M8 12h.01M12 12h.01M16 12h.01M6 16h12"/></svg>'; voiceBtn.title = '切换语音'; } }; const voiceRecordBtn = document.getElementById('voiceRecordBtn'); let voiceIsRecording = false; if (voiceRecordBtn) { voiceRecordBtn.addEventListener('click', (e) => { e.preventDefault(); if (!voiceStream) { toast('请先点击 ⌨️ 切换到语音模式'); return; } if (!voiceIsRecording) { try { voiceMediaRecorder = new MediaRecorder(voiceStream, { mimeType: 'audio/webm;codecs=opus', audioBitsPerSecond: 64000 }); const chunks = []; voiceRecordStartTime = Date.now(); voiceCancelled = false; voiceIsRecording = true; voiceRecordBtn.classList.add('recording'); voiceRecordBtn.style.background = '#a08060'; voiceMediaRecorder.ondataavailable = ev => chunks.push(ev.data); voiceMediaRecorder.onstop = () => { clearInterval(voiceRecordTimer); voiceRecordTimer = null; voiceIsRecording = false; voiceRecordBtn.classList.remove('recording'); voiceRecordBtn.innerHTML = '点按说话'; voiceRecordBtn.style.background = ''; if (voiceCancelled) { toast('已取消录音'); return; } const blob = new Blob(chunks, { type: 'audio/webm' }); const reader = new FileReader(); reader.onload = () => { const duration = Math.round((Date.now() - voiceRecordStartTime) / 1000) || 1; const msg = { voice: reader.result, voiceDuration: duration, voiceNote: '', me: true, time: new Date().toISOString(), senderId: 'me', senderName: appData.myProfile.name }; addMessage(currentChatId, msg); appData.lastUserMsgTime[currentChatId] = Date.now(); scheduleReply(currentChatId, getContact(currentChatId)); }; reader.readAsDataURL(blob); }; voiceMediaRecorder.start(); voiceRecordBtn.innerHTML = '<span class="recording-dot"></span>录音中 00:00'; voiceRecordTimer = setInterval(() => { const elapsed = Math.floor((Date.now() - voiceRecordStartTime) / 1000); const mins = Math.floor(elapsed / 60); const secs = elapsed % 60; voiceRecordBtn.innerHTML = '<span class="recording-dot"></span>录音中 ' + String(mins).padStart(2, '0') + ':' + String(secs).padStart(2, '0'); }, 200); } catch (err) { toast('录音启动失败，请重试'); } } else { if (voiceMediaRecorder && voiceMediaRecorder.state === 'recording') { voiceMediaRecorder.stop(); } } }); voiceRecordBtn.addEventListener('pointerdown', (e) => { if (voiceIsRecording) { voiceRecordStartPointerX = e.clientX; } }); document.addEventListener('pointermove', (e) => { if (!voiceIsRecording) return; const dx = e.clientX - voiceRecordStartPointerX; if (dx < -50) { voiceCancelled = true; voiceRecordBtn.innerHTML = '松开取消'; voiceRecordBtn.style.background = '#f44336'; } else { voiceCancelled = false; voiceRecordBtn.style.background = '#a08060'; } }); } document.getElementById('globalAddBtn').onclick = () => { const o = document.createElement('div'); o.className = 'mask show'; const card = document.createElement('div'); card.className = 'pop-card'; card.style.width = '280px'; const header = document.createElement('div'); header.className = 'pop-header'; header.textContent = '添加'; const closeBtn = document.createElement('span'); closeBtn.className = 'close-pop'; closeBtn.textContent = '✕'; closeBtn.onclick = () => o.remove(); header.appendChild(closeBtn); card.appendChild(header); const body = document.createElement('div'); body.className = 'pop-body'; const addFriendBtn = document.createElement('button'); addFriendBtn.className = 'btn-primary'; addFriendBtn.textContent = ' 添加好友'; addFriendBtn.onclick = () => { showInputDialog('好友昵称', '', (n) => { if (n) { const u = { id: 'u_' + Date.now(), name: n, avt: '', bg: '', status: STATUS_LIST[Math.floor(Math.random() * STATUS_LIST.length)], cardGroups: [{ name: '未分组', list: [] }], rule: { min: 30, max: 60, cntMin: 3, cntMax: 5 }, noReply: false, noDisturb: true, allowInitiative: true, soundEnabled: true, replyTime: 86400, muted: false }; appData.users.push(u); markDataChanged(); save(); renderChatList(); renderContactList(); currentChatId = u.id; openChat(); } }); o.remove(); }; const createGroupBtn = document.createElement('button'); createGroupBtn.className = 'btn-primary'; createGroupBtn.textContent = ' 创建群聊'; createGroupBtn.onclick = () => { const ml = [{ id: 'me', name: '我', avt: appData.myProfile.avt }, ...appData.users.map(u => ({ id: u.id, name: u.name, avt: u.avt }))]; if (ml.length === 1) { toast('请先添加好友'); return; } const sb = document.createElement('div'); sb.className = 'mask show'; const card2 = document.createElement('div'); card2.className = 'pop-card'; const header2 = document.createElement('div'); header2.className = 'pop-header'; header2.textContent = '选择成员'; const closeBtn2 = document.createElement('span'); closeBtn2.className = 'close-pop'; closeBtn2.textContent = '✕'; closeBtn2.onclick = () => sb.remove(); header2.appendChild(closeBtn2); card2.appendChild(header2); const body2 = document.createElement('div'); body2.className = 'pop-body'; ml.forEach(m => { const label = document.createElement('label'); const cb = document.createElement('input'); cb.type = 'checkbox'; cb.value = m.id; cb.className = 'gcb'; label.appendChild(cb); if (m.avt) { const img = document.createElement('img'); img.src = m.avt; img.style.width = '24px'; label.appendChild(img); } label.appendChild(document.createTextNode(' ' + m.name)); body2.appendChild(label); body2.appendChild(document.createElement('br')); }); card2.appendChild(body2); const footer = document.createElement('div'); footer.className = 'pop-footer'; const selAllBtn = document.createElement('button'); selAllBtn.className = 'btn'; selAllBtn.textContent = '全选'; selAllBtn.onclick = () => sb.querySelectorAll('.gcb').forEach(c => c.checked = true); const confirmBtn = document.createElement('button'); confirmBtn.className = 'btn-primary'; confirmBtn.textContent = '创建'; confirmBtn.onclick = () => { const s = Array.from(sb.querySelectorAll('.gcb:checked')).map(c => c.value); if (!s.length) { toast('至少选一人'); return; } showInputDialog('群聊名称', '新群聊', (gn) => { if (!gn) return; const ng = { id: 'g_' + Date.now(), name: gn, members: s, avt: '', bg: '', rule: { min: 30, max: 60, cntMin: 3, cntMax: 5 }, noReply: false, noDisturb: true, announcement: '', mutedAll: false, mutedMembers: {} }; appData.groups.push(ng); markDataChanged(); save(); renderChatList(); renderContactList(); currentChatId = ng.id; openChat(); sb.remove(); o.remove(); }); }; footer.appendChild(selAllBtn); footer.appendChild(confirmBtn); card2.appendChild(footer); sb.appendChild(card2); document.body.appendChild(sb); sb.addEventListener('click', e => { if (e.target === sb) sb.remove(); }); }; body.appendChild(addFriendBtn); body.appendChild(createGroupBtn); card.appendChild(body); o.appendChild(card); document.body.appendChild(o); o.addEventListener('click', e => { if (e.target === o) o.remove(); }); applyTransparency(); }; document.querySelectorAll('.nav-item').forEach(b => b.onclick = () => { const nav = b.dataset.nav; if (nav === 'chats' && document.getElementById('chatsPage').classList.contains('active')) return; if (nav === 'contacts' && document.getElementById('contactsPage').classList.contains('active')) return; if (nav === 'discover' && document.getElementById('discoverPage').classList.contains('active')) return; if (nav === 'me' && document.getElementById('mePage').classList.contains('active')) return; if (nav === 'chats') { switchToChatsTab(); renderChatList(); } else if (nav === 'contacts') { switchToContactsTab(); renderContactList(); } else if (nav === 'discover') switchToDiscoverTab(); else if (nav === 'me') { switchToMeTab(); renderMePage(); } }); document.getElementById('pokeBackBtn').onclick = () => { document.getElementById('pokePage').classList.remove('active'); if (_pokeFromPage === 'chat') { document.getElementById('chatPage').classList.add('active'); } else { switchToChatsTab(); renderChatList(); } }; document.getElementById('emojiManagerBackBtn').onclick = () => { document.getElementById('emojiManagerPage').classList.remove('active'); switchToMeTab(); renderMePage(); }; document.getElementById('blockedBackBtn').onclick = () => { document.getElementById('blockedPage').classList.remove('active'); switchToMeTab(); renderMePage(); }; document.getElementById('globalCardsBackBtn').onclick = () => { document.getElementById('globalCardsPage').classList.remove('active'); switchToMeTab(); renderMePage(); }; document.getElementById('userCardsBackBtn').onclick = () => { document.getElementById('userCardsPage').classList.remove('active'); if (_cardsFromPage === 'chat') { document.getElementById('chatPage').classList.add('active'); } else { switchToChatsTab(); renderChatList(); } }; document.getElementById('myBgBackBtn').onclick = () => { document.getElementById('myBgPage').classList.remove('active'); switchToMeTab(); renderMePage(); }; document.getElementById('contactBgBackBtn').onclick = () => { document.getElementById('contactBgPage').classList.remove('active'); document.getElementById('contactDetailPage').classList.add('active'); renderContactDetail(_contactBgUserId); }; document.getElementById('searchBackBtn').onclick = () => { document.getElementById('searchPage').classList.remove('active'); if (currentChatId) { document.getElementById('chatPage').classList.add('active'); } else { switchToChatsTab(); renderChatList(); } }; document.getElementById('musicBackBtn').onclick = () => { document.getElementById('musicPage').classList.remove('active'); if (_musicFromChat && currentChatId) { _musicFromChat = false; document.getElementById('chatPage').classList.add('active'); } else { switchToChatsTab(); } }; document.getElementById('contactDetailBackBtn').onclick = () => { document.getElementById('contactDetailPage').classList.remove('active'); document.getElementById('contactsPage').classList.add('active'); renderContactList(); }; document.getElementById('goMomentsBtn').onclick = () => { openMomentsPage('me', false); }; document.getElementById('goMailboxBtn').onclick = openMailboxPage; document.getElementById('goMusicBtn').onclick = () => { _musicFromChat = false; openMusicPage(); }; document.getElementById('goCompanionBtn').onclick = () => { openCompanionPage(); }; document.getElementById('goBookListsBtn').onclick = () => { openBookListPage(); }; document.getElementById('goMovieListsBtn').onclick = () => { openMovieListPage(); }; document.getElementById('goTarotBtn').onclick = () => { openTarotPage(); }; document.getElementById('goMovieTogetherBtn').onclick = () => { openMovieTogetherPage(); }; document.getElementById('goDatingBtn').onclick = () => { openDatingPage(); }; document.getElementById('datingBackBtn').onclick = () => { document.getElementById('datingPage').classList.remove('active'); switchToDiscoverTab(); }; document.getElementById('gomokuBackBtn').onclick = () => { document.getElementById('gomokuPage').classList.remove('active'); document.getElementById('datingPage').classList.add('active'); renderDatingPage(); }; document.getElementById('colorSaveBackBtn').onclick = () => {document.getElementById('colorSavePage').classList.remove('active');
  ...
  if (chatMessages && !chatMessages.dataset.imgZoomBound) {
    chatMessages.dataset.imgZoomBound = '1';
    chatMessages.addEventListener('click', (e) => {
      const img = e.target.closest('img');
      if (!img) return;
      // 排除语音气泡内的图标等非内容图片
      if (img.closest('.voice-msg-bubble') && img.tagName === 'svg') return;
      e.stopPropagation();
      const src = img.src;
      ...
      bigImg.onerror = () => { bigImg.style.display = 'none'; };
...
// ========== 快捷回复 ==========
let quickReplies = JSON.parse(localStorage.getItem('Mo_quick_replies') || '[]');

function openQuickReplyPage() {
  hideAllPages();
  document.getElementById('quickReplyPage').classList.add('active');
  renderQuickReplyPage();
}

function renderQuickReplyPage() {
  const body = document.getElementById('quickReplyPageContent');
  ...
  if (quickReplies.length === 0) {
    list.innerHTML = '<div style="text-align:center;color:var(--text-secondary);padding:40px;">暂无快捷回复</div>';
  } else {
    quickReplies.forEach((item, idx) => {
      const row = document.createElement('div');
      row.style.cssText = 'display:flex;align-items:center;padding:10px;border-bottom:1px solid var(--border-light);gap:8px;';
      
      const contentSpan = document.createElement('span');
      contentSpan.style.cssText = 'flex:1;font-size:14px;word-break:break-word;cursor:pointer;';
      if (item.type === 'text') {
      ...
      contentSpan.onclick = () => {
        ...
        document.getElementById('quickReplyPage').classList.remove('active');
      ...
      delBtn.style.cssText = 'color:#f44336;font-size:12px;padding:4px 8px;';
      ...
      delBtn.onclick = (e) => {
        ...
        localStorage.setItem('Mo_quick_replies', JSON.stringify(quickReplies));
        renderQuickReplyPage();
      };
  ...
  input.style.cssText = 'flex:1;padding:6px 10px;border-radius:14px;border:1px solid var(--border-light);font-size:14px;';
  ...
  addBtn.onclick = () => {
    ...
    if (!val) return;
    quickReplies.push({ type: 'text', content: val });
    localStorage.setItem('Mo_quick_replies', JSON.stringify(quickReplies));
    input.value = '';
    renderQuickReplyPage();
  };
  ...
  imgBtn.onclick = () => {
    ...
    fileInput.onchange = async (e) => {
      ...
      try {
        ...
        quickReplies.push({ type: 'image', content: b64 });
        localStorage.setItem('Mo_quick_replies', JSON.stringify(quickReplies));
        renderQuickReplyPage();
      } catch { toast('图片添加失败'); }
    };
...
document.getElementById('quickReplyBackBtn').onclick = () => {
  document.getElementById('quickReplyPage').classList.remove('active');
  if (currentChatId) {
    document.getElementById('chatPage').classList.add('active');
  } else {
