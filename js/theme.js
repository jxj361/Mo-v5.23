...
previewWrapper.style.cssText = 'width:180px;height:320px;margin:0 auto 12px;border-radius:20px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.25);display:flex;flex-direction:column;';

// 状态栏
const statusBar = document.createElement('div');
statusBar.style.cssText = 'height:16px;background:rgba(0,0,0,0.1);display:flex;align-items:center;justify-content:space-between;padding:0 8px;font-size:8px;color:white;flex-shrink:0;';
statusBar.innerHTML = '<span>9:41</span><span>🔋📶</span>';

// 标题栏
const titleBar = document.createElement('div');
titleBar.style.cssText = 'height:28px;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:600;flex-shrink:0;';
titleBar.textContent = pageId === 'chats' ? '传讯' : pageId === 'contacts' ? '通讯录' : pageId === 'discover' ? '发现' : '我';

// 内容区
const contentArea = document.createElement('div');
contentArea.style.cssText = 'flex:1;padding:6px;display:flex;flex-direction:column;gap:4px;overflow:hidden;';

// 根据页面类型填充模拟内容
if (pageId === 'chats') {
    contentArea.innerHTML = `
        <div style="display:flex;align-items:center;gap:6px;padding:5px 6px;background:rgba(255,255,255,0.15);border-radius:8px;">
            <div style="width:28px;height:28px;border-radius:6px;background:rgba(255,255,255,0.35);flex-shrink:0;"></div>
            <div style="flex:1;"><div style="height:8px;background:rgba(255,255,255,0.5);border-radius:4px;width:60%;margin-bottom:3px;"></div><div style="height:6px;background:rgba(255,255,255,0.3);border-radius:4px;width:85%;"></div></div>
            <div style="font-size:8px;color:rgba(255,255,255,0.6);">21:30</div>
        </div>
        <div style="display:flex;align-items:center;gap:6px;padding:5px 6px;background:rgba(255,255,255,0.15);border-radius:8px;">
            <div style="width:28px;height:28px;border-radius:6px;background:rgba(255,255,255,0.35);flex-shrink:0;"></div>
            <div style="flex:1;"><div style="height:8px;background:rgba(255,255,255,0.5);border-radius:4px;width:50%;margin-bottom:3px;"></div><div style="height:6px;background:rgba(255,255,255,0.3);border-radius:4px;width:70%;"></div></div>
            <div style="font-size:8px;color:rgba(255,255,255,0.6);">20:15</div>
        </div>
        <div style="display:flex;align-items:center;gap:6px;padding:5px 6px;background:rgba(255,255,255,0.15);border-radius:8px;">
            <div style="width:28px;height:28px;border-radius:6px;background:rgba(255,255,255,0.35);flex-shrink:0;"></div>
            <div style="flex:1;"><div style="height:8px;background:rgba(255,255,255,0.5);border-radius:4px;width:55%;margin-bottom:3px;"></div><div style="height:6px;background:rgba(255,255,255,0.3);border-radius:4px;width:75%;"></div></div>
            <div style="font-size:8px;color:rgba(255,255,255,0.6);">18:42</div>
        </div>
    `;
} else if (pageId === 'contacts') {
    contentArea.innerHTML = `
        <div style="display:flex;align-items:center;gap:6px;padding:5px 6px;">
            <div style="width:28px;height:28px;border-radius:6px;background:rgba(255,255,255,0.3);flex-shrink:0;display:flex;align-items:center;justify-content:center;font-size:12px;">👤</div>
            <div style="font-size:10px;font-weight:500;">联系人 1</div>
        </div>
        <div style="display:flex;align-items:center;gap:6px;padding:5px 6px;">
            <div style="width:28px;height:28px;border-radius:6px;background:rgba(255,255,255,0.3);flex-shrink:0;display:flex;align-items:center;justify-content:center;font-size:12px;">👤</div>
            <div style="font-size:10px;font-weight:500;">联系人 2</div>
        </div>
        <div style="display:flex;align-items:center;gap:6px;padding:5px 6px;">
            <div style="width:28px;height:28px;border-radius:6px;background:rgba(255,255,255,0.3);flex-shrink:0;display:flex;align-items:center;justify-content:center;font-size:12px;">👤</div>
            <div style="font-size:10px;font-weight:500;">联系人 3</div>
        </div>
    `;
} else if (pageId === 'discover') {
    contentArea.innerHTML = `
        <div style="display:flex;align-items:center;gap:6px;padding:5px 6px;">
            <div style="width:28px;height:28px;border-radius:6px;background:rgba(255,255,255,0.25);flex-shrink:0;display:flex;align-items:center;justify-content:center;font-size:12px;">☀️</div>
            <div style="font-size:10px;font-weight:500;">朋友圈</div>
        </div>
        <div style="display:flex;align-items:center;gap:6px;padding:5px 6px;">
            <div style="width:28px;height:28px;border-radius:6px;background:rgba(255,255,255,0.25);flex-shrink:0;display:flex;align-items:center;justify-content:center;font-size:12px;">📬</div>
            <div style="font-size:10px;font-weight:500;">信箱</div>
        </div>
        <div style="display:flex;align-items:center;gap:6px;padding:5px 6px;">
            <div style="width:28px;height:28px;border-radius:6px;background:rgba(255,255,255,0.25);flex-shrink:0;display:flex;align-items:center;justify-content:center;font-size:12px;">🎵</div>
            <div style="font-size:10px;font-weight:500;">一起听</div>
        </div>
    `;
} else {
    contentArea.innerHTML = `
        <div style="display:flex;flex-direction:column;align-items:center;padding:10px 0;">
            <div style="width:44px;height:44px;border-radius:10px;background:rgba(255,255,255,0.35);margin-bottom:4px;"></div>
            <div style="height:8px;background:rgba(255,255,255,0.5);border-radius:4px;width:35%;margin-bottom:3px;"></div>
            <div style="height:6px;background:rgba(255,255,255,0.3);border-radius:4px;width:25%;"></div>
        </div>
        <div style="display:flex;gap:6px;padding:3px 6px;">
            <div style="flex:1;height:20px;background:rgba(255,255,255,0.2);border-radius:6px;"></div>
            <div style="flex:1;height:20px;background:rgba(255,255,255,0.2);border-radius:6px;"></div>
        </div>
        <div style="display:flex;gap:6px;padding:3px 6px;">
            <div style="flex:1;height:20px;background:rgba(255,255,255,0.2);border-radius:6px;"></div>
...
bottomNav.style.cssText = 'height:32px;background:rgba(255,255,255,0.15);display:flex;align-items:center;justify-content:space-around;flex-shrink:0;font-size:8px;';
const navItems = [
    { icon: '💬', label: '传讯' },
    { icon: '👥', label: '通讯录' },
    { icon: '🧭', label: '发现' },
...
navItems.forEach(item => {
    ...
    navItem.style.cssText = 'display:flex;flex-direction:column;align-items:center;gap:1px;';
    navItem.innerHTML = `<span style="font-size:12px;">${item.icon}</span><span style="font-size:7px;">${item.label}</span>`;
    bottomNav.appendChild(navItem);
});

previewWrapper.appendChild(statusBar);
previewWrapper.appendChild(titleBar);
previewWrapper.appendChild(contentArea);
previewWrapper.appendChild(bottomNav);
tabContent.appendChild(previewWrapper);

// 更新预览
const updatePreview = () => {
    const s = appData.navBgs[pageId] || { mode: 'followTheme', color: '#f5f3f0', image: '', overlayOpacity: 40 };

    previewWrapper.style.background = '';
    previewWrapper.style.backgroundImage = '';
    previewWrapper.style.backgroundColor = '';

    if (s.mode === 'followTheme') {
        previewWrapper.style.backgroundColor = appData.themeColor || '#c5a47e';
    } else if (s.mode === 'color') {
        previewWrapper.style.backgroundColor = s.color || '#f5f3f0';
    } else if (s.mode === 'image' && s.image) {
        previewWrapper.style.backgroundImage = `url(${s.image})`;
        previewWrapper.style.backgroundSize = 'cover';
        previewWrapper.style.backgroundPosition = 'center';
    }

    const opacity = (s.overlayOpacity || 40) / 100;
    if (s.mode === 'image' && s.image) {
      if (opacity >= 0.99) {
        contentArea.style.background = 'transparent';
      } else {
        contentArea.style.background = `rgba(255,255,255,${opacity})`;
      }
    } else {
      contentArea.style.background = 'transparent';
    }
