...
function sendMessage(text, image = null) {
    ...
    input.innerHTML = ''; input.placeholder = '输入消息...'; input.blur();
}
...
function openChatMenu() {
    ...
    if (target.members) {
        ...
        const btns = [
            ...
            { text: '陪伴系统', icon: 'clock', action: () => { mask.classList.remove('show'); openCompanionPage(); } },
            { text: '一起看电影', icon: 'video', action: () => { mask.classList.remove('show'); openMovieTogetherPage(); } },
            { text: '快捷回复', icon: 'message-circle', action: () => { 
    mask.classList.remove('show'); 
    // 直接切换页面，不依赖 openQuickReplyPage 函数
    document.querySelectorAll('.page').forEach(p => p.classList.remove('active'));
    const qp = document.getElementById('quickReplyPage');
    if (qp) {
        qp.classList.add('active');
        if (typeof renderQuickReplyPage === 'function') renderQuickReplyPage();
    }
} },
